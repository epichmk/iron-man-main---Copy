    ...
    <section className="relative w-full h-screen snap-start overflow-hidden bg-[var(--page-bg)]" dir="rtl">
      ...
      <div className="relative z-10 w-full h-full flex flex-col justify-end">
        ...
        <div className="absolute top-20 inset-x-0 z-30 flex flex-col items-center text-center px-6 pointer-events-none">
          ...
          <p className="font-sans text-[11px] text-[var(--text-tertiary)] tracking-wide uppercase">
        ...
        <div className="w-full h-full pb-10 flex flex-col items-center justify-center pt-24">
          <AnimatePresence mode="popLayout" initial={false} custom={direction}>
            ...
            >
              {/* Media Card */}
              <div className="relative w-full max-w-[320px] aspect-[4/5] rounded-[2rem] overflow-hidden border border-[var(--border-subtle)] shadow-[0_20px_50px_var(--shadow-color)]">
                <Image
                  src={activeCard.image}
                  alt={activeCard.title}
                  fill
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[var(--page-bg)] via-black/40 to-transparent" />
                
                {/* Card Content */}
                <div className="absolute inset-x-0 bottom-0 p-6 flex flex-col items-start text-right">
                  <h4 className="font-heading text-2xl font-black text-[var(--text-primary)] mb-1">
                    {activeCard.title}
                  </h4>
                  <p className="font-sans text-xs text-[var(--text-secondary)] font-light mb-4">
                    {activeCard.subtitle}
                  </p>
                  <Link 
                    href={activeCard.link}
                    className="inline-flex items-center justify-center px-6 py-2.5 rounded-full bg-[var(--surface-hover)] hover:bg-[var(--surface-hover)] border border-[var(--border-medium)] text-[var(--text-primary)] text-[11px] font-bold tracking-widest uppercase transition-all backdrop-blur-md"
                  >
                    استعرض الألبوم
                  </Link>
                </div>
