...
export function MobileTestimonials() {
  return (
    <section className="relative w-full h-screen snap-start overflow-hidden bg-[var(--page-bg)]" dir="rtl">
            ...
            >
              ...
              >
                {[...colItems, ...colItems].map((t, idx) => (
                  <div
                    key={idx}
                    className="w-full p-6 rounded-3xl bg-[var(--overlay-bg)] border border-[var(--border-subtle)] border-t-white/20 flex flex-col shadow-[0_20px_40px_var(--shadow-color)] backdrop-blur-md"
                  >
                    ...
                    <p className="font-heading text-[var(--text-primary)] text-[13px] leading-snug font-medium text-center">
                      "{t.text}"
                    </p>
                  </div>
                ))}
      ...
      {/* Fade Overlays */}
      <div className="absolute top-0 inset-x-0 h-40 bg-gradient-to-b from-[var(--page-bg)] to-transparent z-20 pointer-events-none" />
      <div className="absolute bottom-0 inset-x-0 h-40 bg-gradient-to-t from-[var(--page-bg)] to-transparent z-20 pointer-events-none" />
    </section>
  );
}
