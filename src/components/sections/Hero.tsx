          ...
          children.forEach((child, idx) => {
            ...
            if (idx === 3 && op > 0) {
              ...
              if (stat1) {
                ...
                stat1.setAttribute('style', `${isDark ? `text-shadow: 0 0 ${op * 20}px rgba(212,230,22,0.8);` : ''} transform: scale(${1 + (1-op)*0.5})`);
              }
              if (stat2) {
                const val = Math.min(30, Math.floor(op * 30));
                stat2.textContent = `+${val}`;
                stat2.setAttribute('style', `${isDark ? `text-shadow: 0 0 ${op * 20}px rgba(212,230,22,0.8);` : ''} transform: scale(${1 + (1-op)*0.5})`);
              }
              if (stat3) {
                const chars = ['I', 'X', '3'];
                const revealIdx = Math.floor(op * 3);
                stat3.textContent = chars.slice(0, revealIdx + 1).join('');
                stat3.setAttribute('style', `${isDark ? `text-shadow: 0 0 ${op * 30}px rgba(212,230,22,1);` : ''} transform: scale(${1 + (1-op)*0.5})`);
              }
            }
          });
        }
    ...
    <section ref={sectionRef} className="scroll-animation relative">
      ...
      >
        ...
        >
          ...
          <h1 
            className="font-sans text-[2.5rem] font-bold tracking-normal text-[var(--text-primary)] md:text-5xl lg:text-6xl md:whitespace-nowrap leading-[1.3] md:leading-[1.15] lg:leading-[1.15] mx-auto md:mx-0 w-full md:w-auto dark:drop-shadow-md"
          >
            <span className="text-blue-400 inline-block">أول</span> طبيبة أطفال أنابيب في <span className="text-accent">اليمن</span>
          </h1>
          <p className="w-[90%] mx-auto md:mx-0 md:w-auto md:max-w-none font-sans text-base leading-relaxed text-[var(--text-secondary)] md:text-lg dark:drop-shadow" dir="rtl">
        ...
        >
          <div ref={bigLeftTextRef} className="flex flex-col gap-3 md:gap-3 items-center md:items-start w-full">
            ...
            <h2 className="opacity-0 translate-y-8 font-sans font-semibold tracking-normal leading-[1.2] text-[var(--text-primary)] text-2xl md:text-4xl lg:text-[2.75rem] dark:drop-shadow-md">
              <div className="animate-float">بعض الأحلام... لا يمكن لنا ان نعيش من غير تحقيقها</div>
            </h2>
            <p className="opacity-0 translate-y-8 max-w-[50ch] font-sans text-base md:text-lg leading-relaxed text-[var(--text-secondary)] dark:drop-shadow md:max-w-[450px]" dir="rtl">
            ...
            <div className="opacity-0 translate-y-8 hidden md:flex flex-row w-full justify-start gap-4 mt-2">
              <Link
                href="/booking"
                className="inline-flex items-center justify-center rounded-full bg-accent text-[#020817] px-6 py-3 font-sans text-sm font-bold shadow-[0_0_20px_rgba(212,230,22,0.3)] transition-all hover:bg-white hover:shadow-[0_0_30px_var(--text-tertiary)] flex-none"
              >
                إحجز موعد الآن
        ...
        <div className="absolute inset-x-0 bottom-0 z-10">
          <div className="mx-6 mb-3 h-px bg-[var(--surface-hover)] md:mx-10">
            <div
              ref={progressFillRef}
        ...
        {DIALOGUES.map((d, i) => {
          return (
            ...
            >
              <figure className="relative w-full h-[280px] md:h-[350px] overflow-hidden rounded-3xl shadow-[0_20px_50px_var(--shadow-color)] ">
                {/* Background Image */}
                ...
                <div className="relative z-10 flex flex-col justify-end h-full p-6 md:p-8 text-center md:text-right" dir="rtl">
                  <blockquote className="font-sans text-xl md:text-2xl font-bold leading-tight tracking-tight text-[var(--text-primary)] dark:drop-shadow-md">
                    {d.quote.split('\n')[0]}
                  </blockquote>
                  <p className="mt-3 font-sans text-sm md:text-base text-[var(--text-secondary)] whitespace-pre-line leading-relaxed dark:drop-shadow">
