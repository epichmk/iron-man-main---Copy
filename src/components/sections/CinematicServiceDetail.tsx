...
function Immersive3DCard({ step, idx }: { step: { step: number; title: string; description: string }, idx: number }) {
  ...
  return (
    <ScrollReveal className="relative flex-1 min-w-[200px] group">
      ...
      >
        <div className="absolute inset-0 bg-[var(--surface-elevated)]/80 backdrop-blur-2xl  shadow-[0_30px_60px_var(--shadow-color)] group-#d4e616]/50 transition-all duration-700" style={{ transform: "translateZ(-30px)" }} />
        ...
        <div className="relative p-6 md:p-8 h-full flex flex-col justify-start transform-gpu transition-all duration-500" style={{ transform: "translateZ(40px)" }}>
          <div className="flex items-center justify-between mb-6 pb-4 border-b border-[var(--border-subtle)]">
            <span className="text-accent font-bold text-3xl dark:drop-shadow-[0_0_20px_rgba(212,230,22,0.4)] font-mono">0{step.step}</span>
            <span className="text-xs font-mono text-muted tracking-[0.2em] uppercase">المرحلة {String(step.step).padStart(2, '0')}</span>
          </div>
          <motion.h3 initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="text-xl font-bold text-[var(--text-primary)] mb-3 leading-tight group-hover:text-accent transition-colors duration-500">{step.title}</motion.h3>
          <motion.p initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }} className="text-[var(--text-tertiary)] text-sm leading-[1.8] font-light group-hover:text-zinc-200 transition-colors duration-500">{step.description}</motion.p>
        </div>
...
function BenefitRow({ benefit, idx }: { benefit: { label: string; value: string }, idx: number }) {
  ...
  return (
    <motion.div
      ...
      className="group relative w-full border-t border-[var(--border-subtle)] #d4e616] transition-colors duration-700 py-6 md:py-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 cursor-default overflow-hidden"
    >
      ...
      <span className="absolute right-0 md:right-8 top-1/2 -translate-y-1/2 text-[5rem] md:text-[7rem] font-bold text-[var(--text-muted-light)] group-hover:text-accent/[0.05] transition-colors duration-1000 pointer-events-none -z-20 leading-none font-mono">
        0{idx + 1}
      </span>

      <div className="flex-1 max-w-4xl pr-0 md:pr-6 relative z-10">
        <span className="text-accent font-mono text-[10px] md:text-xs tracking-[0.3em] uppercase block mb-2 opacity-70 group-hover:opacity-100 transition-opacity">
          [ {benefit.label} ]
        </span>
        <h3 className="text-xl md:text-2xl lg:text-3xl font-bold text-[var(--text-primary)] leading-[1.3] group-hover:translate-x-4 transition-transform duration-700 ease-out text-balance">
          {benefit.value}
        </h3>
      </div>
      ...
      <div className="md:w-1/4 flex justify-start md:justify-end relative z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
        <div className="w-12 h-12 rounded-full #d4e616] flex items-center justify-center bg-accent/10">
           <ArrowUpLeft className="w-4 h-4 text-accent" />
        </div>
      </div>
    </motion.div>
  );
...
function EditorialStats({ stats }: { stats: { label: string; value: string }[] }) {
  ...
  return (
    <ScrollReveal direction="up" className="w-full relative py-8 mt-4 border-t border-[var(--border-subtle)]">
       <div className="flex flex-wrap items-center gap-10 md:gap-16 relative z-10">
         {stats.map((stat, idx) => (
           <div key={idx} className="flex flex-col relative group">
             <span className="text-xs text-accent/70 font-mono tracking-[0.2em] uppercase mb-2 group-hover:text-accent transition-colors">
               {stat.label}
             </span>
             <span className="text-2xl md:text-4xl font-bold text-[var(--text-primary)] tracking-wide leading-none group-hover:dark:drop-shadow-[0_0_20px_rgba(212,230,22,0.4)] transition-all duration-500 font-mono inline-block" dir="ltr">
               {stat.value}
             </span>
             <div className="h-[2px] w-0 bg-accent group-hover:w-full transition-all duration-500 mt-4 opacity-50" />
           </div>
         ))}
       </div>
    </ScrollReveal>
  ...
  return (
    <ScrollReveal direction="up" className="w-full relative py-8 mt-4 border-t border-[#0066FF]/20">
       ...
       <div className="flex flex-col gap-4">
         {tech.map((t, idx) => (
           <div key={idx} className="group relative flex items-center justify-between p-5 rounded-2xl  bg-[var(--surface-elevated)] hover:bg-[#0066FF]/10 #0066FF]/40 transition-all duration-500 overflow-hidden shadow-lg">
             ...
             <div className="relative z-10 flex flex-col pr-2">
               <motion.span initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-xl font-bold text-[var(--text-primary)] group-hover:text-[#0066FF] transition-colors mb-1 font-mono tracking-wide">{highlightKeywords(t.name)}</motion.span>
               <motion.span initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="text-base text-[var(--text-tertiary)] group-hover:text-[var(--text-secondary)] transition-colors">{highlightKeywords(t.description)}</motion.span>
             </div>
             <div className="relative z-10 w-12 h-12 rounded-full  flex items-center justify-center group-#0066FF]/50 group-hover:bg-[#0066FF]/20 transition-all duration-500 shadow-inner">
                <span className="text-[var(--text-primary)] group-hover:text-[#0066FF] font-mono text-sm block" dir="ltr">0{idx+1}</span>
             </div>
           </div>
         ))}
       </div>
...
export function CinematicServiceDetail({ service }: { service: typeof servicesData[0] }) {
  ...
  const animationVariants = [
    ...
    { initial: { opacity: 0, scale: 1.1 }, animate: { opacity: 1, scale: 1 }, transition: { duration: 1.2, ease: [0.16, 1, 0.3, 1] as const } },
  ...
  return (
    <div 
      ref={containerRef} 
      className="relative w-full min-h-screen bg-[var(--page-bg)] overflow-clip selection:bg-accent/30 selection:text-[var(--text-primary)] pb-24"
      onPointerMove={handlePointerMove}
      dir="rtl" // STRICT RTL ROOT
    >
      <div className="fixed top-0 right-0 w-[60vw] h-[60vh] bg-[radial-gradient(ellipse_at_top_right,var(--surface-glass)_0%,transparent_70%)] pointer-events-none z-0" />
      ...
      {/* ═══════════════════════════════════════════
          ...
          ═══════════════════════════════════════════ */}
      <section className="relative z-10 w-full h-screen snap-start flex flex-col items-center justify-center overflow-hidden bg-[var(--page-bg)]">
        ...
        >
          <Image 
            ...
            fill 
            sizes="100vw"
            className={`absolute inset-0 w-full h-full object-cover opacity-40 mix-blend-screen ${service.imagePosition || 'object-center'} hide-in-light`}
            quality={100}
            priority
          />
          {/* Light Mode Image */}
          <Image 
            src={`/services/lightmode/${service.id}_light.jpg`}
            alt={service.title} 
            fill 
            sizes="100vw"
            className={`absolute inset-0 w-full h-full object-cover opacity-40 mix-blend-multiply ${service.imagePosition || 'object-center'} hide-in-dark`}
            quality={100}
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[var(--page-bg)]/80 via-[var(--page-bg)]/50 to-[var(--page-bg)]" />
        </motion.div>
        ...
        <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 2, ease: "easeOut" }} className="absolute inset-0 flex items-center justify-center pointer-events-none select-none z-[1]">
          <h1 className="text-[10rem] md:text-[18rem] lg:text-[22rem] font-bold text-[var(--text-primary)] whitespace-nowrap tracking-tighter opacity-[0.02]">
            NMC
          </h1>
        </motion.div>
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-accent/10 blur-[150px] rounded-full pointer-events-none z-[1]" />
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-accent font-mono tracking-[0.3em] uppercase text-[10px] font-bold">الخدمات التخصصية الدقيقة</span>
            <div className="w-16 h-[1px] bg-gradient-to-r from-[#d4e616] to-transparent" />
          </div>

          <h2 className="text-4xl md:text-5xl lg:text-7xl font-black text-[var(--text-primary)] mb-6 dark:drop-shadow-2xl leading-[1.2] text-balance tracking-tight">
            {service.title}
          </h2>
          ...
          {service.stats && (
            <div className="flex flex-wrap items-center justify-center gap-4 w-full max-w-4xl">
              {service.stats.map((stat, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 + i * 0.1, duration: 0.8 }} className="flex flex-col items-center gap-1 px-6 py-4 bg-[var(--surface-elevated)]  rounded-2xl min-w-[140px] backdrop-blur-md shadow-2xl">
                  <span className="text-2xl md:text-3xl font-bold text-[var(--text-primary)] font-mono" dir="ltr">{stat.value}</span>
                  <span className="text-[10px] font-mono tracking-wide text-accent">{stat.label}</span>
                </motion.div>
              ))}
            </div>
          )}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-30">
          <span className="font-mono text-[9px] tracking-[0.2em] uppercase text-[var(--text-muted-light)]">استكشف التفاصيل</span>
      ...
      <main className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-12 pb-32 pt-10">
        ...
        {extendedDesc.length > 0 && (
          <div className="w-full relative py-20 mb-32 border-t border-[#0066FF]/20 mt-10">
            <div className="flex flex-col items-center mb-10 relative z-10 text-center">
              ...
              <h2 className="text-4xl md:text-5xl font-black text-[var(--text-primary)] tracking-tight">فلسفة التقنية</h2>
            </div>
              ...
              <div className="flex flex-col gap-[40vh] pb-[20vh] -mt-[30vh] relative z-10">
                {extendedDesc.map((p: string, idx: number) => (
                  <motion.div 
                    ...
                    className="p-8 md:p-14 bg-[var(--page-bg)]/60 backdrop-blur-2xl #0066FF]/20 rounded-[3rem] shadow-[0_40px_80px_var(--shadow-color)] text-center transform-gpu"
                  >
        ...
        {service.clientJourney && (
          <div className="w-full relative py-10 mx-auto mb-20 z-20">
            <ScrollReveal direction="up" className="mb-12 flex flex-col items-start relative z-40">
              <span className="text-accent font-mono text-xs tracking-[0.3em] uppercase mb-3">/ هيكلة الرحلة الطبية</span>
              <h2 className="text-3xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">مراحل البروتوكول</h2>
            </ScrollReveal>

            <div className="relative w-full perspective-1200">
              <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-[var(--surface-hover)] hidden lg:block -translate-y-1/2" />
        ...
        <ScrollReveal direction="up" className="w-full relative py-16 mb-20 border-t border-[var(--border-subtle)]">
          <span className="absolute -top-12 right-0 text-accent/5 text-[20rem] font-serif leading-none pointer-events-none">&quot;</span>
          <div className="relative z-10 flex flex-col md:flex-row gap-12 lg:gap-20 items-start">
            <div className="md:w-1/3 shrink-0">
              <span className="text-accent font-mono text-sm tracking-[0.3em] uppercase block mb-3">/ التفاصيل التقنية</span>
              <h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] mb-8">المواصفات التقنية</h2>
              
              {techExtended.length > 0 && (
                <div className="mt-12">
                  <button 
                    onClick={() => setShowTechDetails(!showTechDetails)}
                    className="flex items-center gap-4 text-[#0066FF] font-mono text-sm tracking-widest uppercase hover:text-[var(--text-primary)] transition-colors group"
                  >
                  ...
                  <AnimatePresence>
                    {showTechDetails && (
                      ...
                      >
                        <div className="flex flex-col gap-6 mt-8 p-8 bg-[var(--surface-elevated)] #0066FF]/20 rounded-3xl shadow-[0_0_30px_rgba(0,102,255,0.1)] backdrop-blur-sm relative">
                          <div className="absolute top-0 right-0 w-32 h-32 bg-[#0066FF]/20 blur-[50px]" />
                          {techExtended.map((p: string, idx: number) => (
                             <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: idx * 0.1 }} key={idx} className="text-[var(--text-tertiary)] text-base md:text-lg leading-[1.8] text-balance relative z-10">
                               {highlightKeywords(p)}
                             </motion.p>
                          ))}
                        </div>
            ...
            <div className="md:w-2/3">
              <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-[var(--text-primary)] text-2xl md:text-3xl lg:text-4xl leading-[1.6] md:leading-[1.9] font-light border-r-4 border-[#0066FF] pr-8 drop-shadow-2xl text-balance">
        ...
        <div className="w-full relative mt-20 mb-20 flex flex-col lg:flex-row gap-16 items-start">
          ...
          <div className="w-full lg:w-3/5 flex flex-col">
            <ScrollReveal direction="up" className="mb-12 flex flex-col items-start">
              <span className="text-accent font-mono text-xs tracking-[0.3em] uppercase mb-3">/ المخرجات الأساسية</span>
              <h2 className="text-3xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">المزايا والمخرجات</h2>
            </ScrollReveal>
          ...
          <div className="w-full lg:w-2/5 flex flex-col mt-20 lg:mt-0 relative perspective-1000 sticky top-40">
            <ScrollReveal direction="up" className="w-full">
              <div className="w-full rounded-3xl #0066FF]/20 bg-[var(--page-bg)]/80 backdrop-blur-3xl p-10 shadow-[0_40px_80px_rgba(0,102,255,0.15)] overflow-hidden group transform-gpu transition-all duration-700 hover:rotate-y-2 hover:-rotate-x-2">
                ...
                <div className="relative z-10 flex flex-col h-full justify-between gap-12">
                  <div>
                    <h3 className="text-3xl md:text-4xl font-black text-[var(--text-primary)] leading-[1.2] mb-6">
                      {service.title}
                    </h3>
                    <div className="relative">
                      ...
                      <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.5 }} className="text-base text-[var(--text-secondary)] font-light pr-4 leading-relaxed border-r-2 border-[#0066FF]/20 line-clamp-4 text-balance">
                        {heroDesc}
                      </motion.p>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    {/* Button Core */}
                    <div className="relative flex items-center justify-between md:justify-center gap-4 bg-[var(--page-bg)] px-6 py-4 rounded-full w-full h-full transition-all duration-500">
                      <span className="text-base md:text-lg font-bold text-[var(--text-primary)] group-hover:text-accent transition-colors whitespace-nowrap">
                        حجز استشارة عبر الواتساب
                      </span>
                      <div className="w-10 h-10 shrink-0 rounded-full bg-accent flex items-center justify-center shadow-[0_0_20px_rgba(212,230,22,0.4)] group-hover:scale-110 transition-transform duration-500">
        ...
        {service.faq && service.faq.length > 0 && (
          <ScrollReveal direction="up" className="w-full relative py-20 border-t border-[var(--border-subtle)]">
            <div className="flex flex-col items-center text-center mb-16">
              <span className="text-accent font-mono text-xs tracking-[0.3em] uppercase mb-4 px-6 py-1 #d4e616]/30 rounded-full">/ الأسئلة الشائعة</span>
              <h2 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] mb-6">أسئلة متكررة حول الخدمة</h2>
            </div>
            <div className="max-w-4xl mx-auto flex flex-col">
              {service.faq.map((item: { q: string; a: string }, idx: number) => (
                <div key={idx} className={`group border-b transition-colors duration-500 overflow-hidden ${openFaq === idx ? 'border-[#d4e616]/30 bg-[var(--surface-elevated)]' : 'border-[var(--border-subtle)] #d4e616]/50'}`}>
                  <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} className="w-full text-right py-6 px-4 flex items-center justify-between outline-none">
                    <h4 className={`text-xl md:text-2xl font-bold transition-colors duration-300 ${openFaq === idx ? 'text-accent' : 'text-[var(--text-primary)] group-hover:text-blue-400'}`}>
                      {item.q}
                    </h4>
                    <div className={`w-8 h-8 rounded-full border flex items-center justify-center flex-shrink-0 transition-all duration-300 ${openFaq === idx ? 'border-[#d4e616] bg-accent/10 text-accent' : 'border-[var(--border-medium)] text-[var(--text-tertiary)] group- group-hover:text-blue-400'}`}>
                  ...
                  <AnimatePresence>
                    {openFaq === idx && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.4, ease: "easeInOut" }}>
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 }} className="px-4 pb-8 text-[var(--text-tertiary)] text-lg leading-relaxed font-light">{item.a}</motion.div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
        ...
        {relatedBlogs.length > 0 && (
          <ScrollReveal direction="up" className="w-full relative py-20 border-t border-[var(--border-subtle)]">
            <div className="flex flex-col items-center text-center mb-16">
              ...
              <h2 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] mb-6">مقالات ذات صلة بالخدمة</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {relatedBlogs.map((post) => (
                <Link key={post.id} href={`/blog/${post.id}`} className="group block bg-[var(--surface-elevated)]  rounded-2xl overflow-hidden #d4e616]/50 hover:shadow-[0_0_30px_rgba(212,230,22,0.15)] transition-all duration-500 hover:-translate-y-2">
                  <div className="p-6">
                    <div className="bg-[var(--overlay-bg)] #d4e616]/30 text-accent text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider inline-block mb-4">{post.category}</div>
                    <motion.h3 initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} className="font-heading text-xl text-[var(--text-primary)] font-bold mb-3 group-hover:text-accent transition-colors line-clamp-2">{post.title}</motion.h3>
                    <motion.p initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="font-sans text-[var(--text-tertiary)] text-sm leading-relaxed mb-6 line-clamp-2">{post.excerpt}</motion.p>
                    <div className="flex items-center gap-4 text-[var(--text-muted-light)] font-sans text-xs pt-4 border-t border-[var(--border-subtle)]">
        ...
        <div dir="rtl">
          <ScrollReveal direction="up" className="w-full mt-20 mb-20 pt-16 border-t border-[var(--border-subtle)]">
            ...
            <Link href={`/services/${prevService.id}`} className="group flex items-center gap-6 w-full md:w-1/2 justify-start">
              <div className="w-16 h-16 rounded-full  flex items-center justify-center group-hover:bg-accent/10 group-#d4e616] transition-all duration-300">
                <ArrowRight className="w-6 h-6 text-[var(--text-muted-light)] group-hover:text-accent group-hover:translate-x-2 transition-all duration-300" />
              </div>
              <div>
                <span className="text-accent font-mono text-xs tracking-widest uppercase block mb-1">الخدمة السابقة</span>
                <span className="text-[var(--text-primary)] font-bold text-2xl group-hover:text-accent transition-colors">{prevService.title}</span>
              </div>
            </Link>

            <Link href={`/services/${nextService.id}`} className="group flex items-center gap-6 w-full md:w-1/2 justify-end text-left">
              <div>
                <span className="text-accent font-mono text-xs tracking-widest uppercase block mb-1 text-left">الخدمة التالية</span>
                <span className="text-[var(--text-primary)] font-bold text-2xl group-hover:text-accent transition-colors text-balance">{nextService.title}</span>
              </div>
              <div className="w-16 h-16 rounded-full  flex items-center justify-center group-hover:bg-accent/10 group-#d4e616] transition-all duration-300">
                <ArrowLeft className="w-6 h-6 text-[var(--text-muted-light)] group-hover:text-accent group-hover:-translate-x-2 transition-all duration-300" />
              </div>
            </Link>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-10 border-t border-[var(--border-subtle)]">
            {quickLinks.map((link) => (
              <Link key={link.id} href={`/services/${link.id}`} className="group block  p-6 #d4e616]/30 bg-[var(--surface-elevated)] hover:bg-accent/5 transition-colors">
                <span className="text-[var(--text-muted-light)] font-mono text-xs tracking-wider uppercase block mb-3 group-hover:text-accent">استكشف المزيد</span>
                <h4 className="text-[var(--text-primary)] font-bold text-lg text-balance">{link.title}</h4>
              </Link>
            ))}
          </div>
        </ScrollReveal>
