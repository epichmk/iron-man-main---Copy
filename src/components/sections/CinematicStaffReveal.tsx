...
export function CinematicStaffReveal({
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      ...
      if (mainTextRef.current) {
        ...
        mainTextRef.current.style.filter = ``;
        ...
        for (let i = 0; i < N; i++) {
           ...
           if (tickerItem) {
             if (dist < segment * 0.5) {
               ...
               tickerItem.style.borderColor = "var(--border-subtle)";
             }
           }
  ...
  return (
    <section
      ref={sectionRef}
      id={id}
      className="scroll-animation relative border-t border-[var(--border-subtle)] bg-[var(--page-bg)]"
      style={{ height: scrollHeight || "300vh", marginBottom: "-100dvh" }}
    >
      <div
        className="sticky top-0 min-h-screen w-full overflow-hidden bg-[var(--page-bg)] z-0"
      ...
      >
        ...
        <div ref={rightTitleContainerRef} className="pointer-events-none absolute right-6 inset-y-0 z-50 md:right-16 flex flex-col justify-center">
          <div ref={mainTextRef} className="flex flex-col items-end border-r-4 border-blue-500/60 pr-6 md:pr-8 text-right w-max max-w-[90vw] md:max-w-[46ch]" style={{ willChange: "transform, opacity, filter", backfaceVisibility: "hidden" }}>
            {id !== 'cinematic-1' && (
              <div className="drop-shadow-[0_2px_10px_var(--shadow-color)] pointer-events-auto mb-5 md:mb-6">
            ...
            <p className="max-w-[42ch] font-sans text-sm md:text-lg leading-[1.8] text-[var(--text-secondary)] dark:drop-shadow-[0_5px_10px_rgba(0,0,0,1)] font-bold pointer-events-auto">
              {description}
            </p>
          </div>
        </div>
          ...
          {staffData?.map((staff, i) => (
             ...
             >
                <div className="relative z-20 flex flex-col items-start justify-center max-w-sm ml-6 mb-24 md:mb-0 pointer-events-auto">
                   ...
                   <h3 className="font-sans text-5xl md:text-6xl font-black text-[var(--text-primary)] mb-6 leading-[1.1] tracking-tighter dark:drop-shadow-[0_10px_30px_var(--shadow-color)]">
                     {staff.name}
                   </h3>
                   <div className="w-12 h-px bg-[var(--border-strong)] mb-6" />
                   <p className="font-sans text-base text-[var(--text-secondary)] font-light leading-relaxed dark:drop-shadow-[0_5px_10px_rgba(0,0,0,1)] text-right">
                ...
                <div className="absolute bottom-0 left-0 w-full md:w-[80%] h-[70vh] md:h-[90vh] pointer-events-none">
                   <Image
                     ...
                     fill
                     quality={100}
                     className="object-contain object-bottom md:object-left-bottom drop-shadow-[0_0_50px_var(--shadow-color)]"
                   />
                   <div className="absolute inset-x-0 bottom-0 h-1/4 bg-gradient-to-t from-[var(--page-bg)] via-black/30 to-transparent" />
                </div>
             </div>
          ))}
        </div>
        ...
        {staffData && staffData.length > 0 && (
          <div className="absolute inset-y-0 left-0 w-32 md:w-48 z-40 pointer-events-none overflow-hidden ml-6 md:ml-12 flex flex-col items-center justify-center">
             ...
             {/* Center Indicator */}
             <div className="absolute top-1/2 left-0 right-0 h-px bg-[var(--surface-hover)] -translate-y-1/2" />
             ...
             >
               {staffData.map((staff, i) => (
                 <div 
                   ...
                   style={{ opacity: 0.4, transform: "scale(0.9)", filter: "grayscale(100%)", borderColor: 'var(--border-subtle)', pointerEvents: "auto" }}
                 >
                    <Image src={staff.image} alt={staff.name} fill className="object-cover object-top opacity-90" />
                    <div className="absolute inset-0 shadow-[inset_0_0_20px_var(--shadow-color)] pointer-events-none" />
                 </div>
               ))}
             </div>
          </div>
