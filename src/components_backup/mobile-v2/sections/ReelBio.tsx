      ...
      <div className={`absolute inset-x-0 bottom-0 h-[60vh] flex flex-col items-center justify-end z-10 pointer-events-none transition-opacity duration-1000 ${isActive ? 'opacity-100' : 'opacity-0'}`}>
        <div className="relative w-full h-full">
          <Image 
            ...
            unoptimized
          />
          <div className="absolute inset-x-0 bottom-0 h-[30vh] bg-gradient-to-t from-black via-black/80 to-transparent z-10 pointer-events-none" />
        </div>
      </div>
      ...
      >
          ...
          <div className="relative z-10 min-h-[160px] flex items-center justify-center w-full">
            {DIALOGUES.map((dialogue, index) => {
              ...
              return (
                ...
                >
                  <h3 className="text-xl sm:text-2xl font-black text-white leading-[1.3] tracking-tight mb-2" style={{ textShadow: "0 10px 20px rgba(0,0,0,0.8)" }}>
                    <span className="text-blue-400 font-serif text-3xl leading-none mr-1">"</span>
                    {dialogue.quote.split('\n')[0]}
                  </h3>
                  <p className="text-[11px] sm:text-xs text-zinc-300 font-bold leading-[1.8] px-2" style={{ textShadow: "0 5px 10px rgba(0,0,0,0.8)" }}>
                    {dialogue.quote.split('\n').slice(1).join(' ')}
                  </p>
                </div>
              );
        ...
        <div className="flex flex-col items-center text-center z-30">
          <div className="flex flex-col items-center gap-3 mb-6">
            ...
            <div className={`transition-all duration-700 delay-200 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} flex items-center gap-4 bg-black/60 rounded-full px-5 py-2 border border-white/10 backdrop-blur-md`}>
            ...
            <div className={`mt-1 transition-all duration-700 delay-300 ${isActive ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              <span className="text-[10px] font-bold tracking-widest text-zinc-300 bg-black/40 px-4 py-1.5 rounded-full border border-white/10 backdrop-blur-sm shadow-[0_5px_15px_rgba(0,0,0,0.8)]">
