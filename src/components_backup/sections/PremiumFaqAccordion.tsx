  ...
  return (
    <div className="w-full relative py-24 bg-transparent">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row gap-16">
        ...
        <motion.div 
          initial={{ opacity: 0, rotateY: 30, y: 100, filter: "blur(20px)" }}
        ...
        >
          <div className="sticky top-32">
            <span className="text-[#d4e616] font-mono text-xs tracking-[0.3em] uppercase mb-4 block">/ FAQ</span>
            <h2 className="text-5xl md:text-7xl font-bold text-white leading-tight mb-6 dark:drop-shadow-2xl">الأسئلة<br/>الشائعة</h2>
            <p className="text-zinc-400 font-sans text-lg">كل ما تحتاج لمعرفته حول رحلة العلاج والتقنيات المستخدمة في مركزنا.</p>
          </div>
        </motion.div>
          ...
          {faqData.map((section, sIdx) => (
            ...
            >
              <div className="overflow-hidden">
                <motion.h3 
                  initial={{ opacity: 0, letterSpacing: "1em", filter: "blur(10px)", y: 20 }}
                  whileInView={{ opacity: 0.8, letterSpacing: "0.1em", filter: "blur(0px)", y: 0 }}
                  viewport={{ once: false, margin: "-100px" }}
                  transition={{ duration: 1.5, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
                  className="font-mono text-[#0066FF] text-sm uppercase mb-4" 
                  dir="ltr"
                >
                ...
                {section.items.map((faq, fIdx) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className={`group relative transition-colors duration-500 overflow-hidden ${isOpen ? 'bg-white/[0.02]' : 'hover:bg-white/[0.01]'}`}
                    >
                      ...
                      <motion.div 
                        ...
                        className={`absolute bottom-0 right-0 w-full h-[1px] origin-right transition-colors duration-500 z-10 ${isOpen ? 'bg-[#d4e616]/50' : 'bg-white/10 group-hover:bg-[#d4e616]/50'}`}
                      />
                      <button 
                        onClick={() => toggleOpen(id)}
                        className="w-full text-right py-6 px-4 flex items-center justify-between outline-none"
                      >
                        <h4 className={`text-xl md:text-2xl font-bold transition-colors duration-300 ${isOpen ? 'text-[#d4e616]' : 'text-white group-hover:text-blue-400'}`}>
                          {faq.q}
                        </h4>
                        <div className={`w-8 h-8 rounded-full border flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isOpen ? 'border-[#d4e616] bg-[#d4e616]/10 text-[#d4e616]' : 'border-white/20 text-white/50 group- group-hover:text-blue-400'}`}>
                      ...
                      <AnimatePresence>
                        {isOpen && (
                          ...
                          >
                            <div className="px-4 pb-8 text-zinc-400 text-lg leading-relaxed font-light">
                              {faq.a}
                            </div>
                          </motion.div>
                        )}
