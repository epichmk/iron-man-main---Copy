...
export function CinematicBlogPost({ post }: CinematicBlogPostProps) {
  ...
  return (
    <div className="relative w-full bg-[#000814] selection:bg-[#d4e616]/30 selection:text-white" dir="rtl">
      {/* Progress */}
      <motion.div className="fixed top-0 left-0 w-full h-1 bg-[#d4e616] origin-left z-50 mix-blend-difference" style={{ width: pw }} />
      ...
      {/* ══════════════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════════════ */}
      <section ref={heroRef} className="relative w-full h-screen overflow-hidden border-b border-white/5 bg-[#000814]">
        ...
        <motion.div className="absolute inset-0 opacity-30 pointer-events-none mix-blend-screen" style={{ y: heroImgY, scale: heroImgScale }}>
          ...
          {!post.category.includes("IVF") && !post.category.includes("Male") && (
            <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
              ...
              <div className="absolute bottom-1/4 -right-1/4 w-[60vw] h-[60vw] bg-[#d4e616]/5 rounded-full blur-[100px]" />
        ...
        <div className="absolute inset-0 z-[1] bg-gradient-to-t from-[#000814] via-[#000814]/80 to-transparent" />
        <div className="absolute inset-0 z-[1] bg-gradient-to-l from-[#000814] via-transparent to-transparent" />
        ...
        <motion.div className="relative z-10 h-full flex flex-col justify-end pb-16 md:pb-24 px-6 md:px-16 lg:px-24 max-w-[1400px] mx-auto" style={{ y: heroTextY, opacity: heroOpacity }}>
          {/* Top Bar */}
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/10 pb-6">
            <div className="flex items-center gap-4">
              ...
              <span className="text-zinc-500 font-mono text-[10px] tracking-[0.2em]">{post.readTime}</span>
            </div>
            <Link href="/blog" className="text-zinc-500 hover:text-white font-mono text-[10px] tracking-[0.2em] uppercase transition-colors flex items-center gap-2">
          ...
          <motion.h1
            ...
            className="text-[clamp(2.5rem,5vw,5.5rem)] font-black text-white leading-[1.1] tracking-tight mb-8 max-w-[1200px]"
          >
            {post.title}
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }} className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <p className="text-zinc-400 text-base md:text-xl font-light max-w-[600px] leading-[1.8] text-balance">
              {post.excerpt}
            </p>
            <div className="text-left md:text-right border-t border-white/10 pt-4 md:border-t-0 md:pt-0 md:border-r md:pr-6">
      ...
      {sections.map((section, index) => {
              ...
              <div className="max-w-[1400px] mx-auto px-6 md:px-16 lg:px-24">
                ...
                >
                  <div className="lg:w-1/3">
                    ...
                    <h2 className="text-[2.5rem] md:text-[3.5rem] font-black text-white leading-[1.1] tracking-tight">
                      {section.title}
                    </h2>
                  </div>
                  <div className="lg:w-2/3">
                    <p className="text-zinc-300 text-lg md:text-xl leading-[2] font-light max-w-[800px] whitespace-pre-wrap">
                      {section.body}
                    </p>
                  </div>
                </motion.div>
        ...
        if (index % 2 !== 0) {
          return (
            <section key={index} className="relative w-full py-32 border-y border-white/5 overflow-hidden">
              <div className="max-w-[1400px] mx-auto px-6 md:px-16 lg:px-24">
                <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="mb-20">
                  ...
                  <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight max-w-4xl leading-[1.3]">
                    {section.title}
                  </h2>
                </motion.div>
                
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16 relative">
                  <div className="hidden md:block absolute top-0 bottom-0 right-[25%] w-[1px] bg-white/5" />
                  ...
                  <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="md:col-span-3">
                    <div className="w-16 h-16 #d4e616]/30 rounded-none flex items-center justify-center mb-6">
                      <span className="text-[#d4e616] font-mono text-xl font-black">0{index + 1}</span>
                    </div>
                  </motion.div>
                  ...
                  <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }} className="md:col-span-9 md:pr-8">
                    <p className="text-zinc-400 text-lg leading-[2.2] font-light whitespace-pre-wrap max-w-3xl">
                      {section.body}
                    </p>
                  </motion.div>
                </div>
          ...
          <section key={index} className="relative w-full py-32 md:py-48 overflow-hidden">
            <div className="max-w-[1400px] mx-auto px-6 md:px-16 lg:px-24 text-center">
              <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1 }}>
                ...
                <h2 className="text-[2rem] md:text-[4rem] font-black text-white leading-[1.2] tracking-tight text-balance max-w-5xl mx-auto">
                  {section.title}
                </h2>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.3 }} className="mt-16 max-w-3xl mx-auto text-right border-r-2 border-[#d4e616] pr-8">
                <p className="text-zinc-300 text-lg leading-[2] font-light whitespace-pre-wrap">
                  {section.body}
                </p>
              </motion.div>
            </div>
      ...
      {/* ══════════════════════════════════════════════════════════
          RELATED ARTICLES
          ══════════════════════════════════════════════════════════ */}
      <section className="relative w-full py-24 border-t border-white/5 bg-[#000814]">
        <div className="max-w-[1400px] mx-auto px-6 md:px-16 lg:px-24">
          <div className="flex items-center justify-between mb-12">
            <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight">
              استكشف المزيد
            </h3>
            <Link href="/blog" className="text-zinc-500 hover:text-[#d4e616] font-mono text-[10px] tracking-[0.2em] uppercase transition-colors flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {blogData.filter(p => p.id !== post.id).slice(0, 3).map((relatedPost, i) => (
              <Link href={`/blog/${relatedPost.id}`} key={relatedPost.id} className="group block  bg-[#000814] #d4e616]/30 transition-all duration-500 p-6">
                <div className="flex flex-col h-full justify-between gap-6">
                  <div>
                    ...
                    <h4 className="text-white font-bold text-lg leading-tight group-hover:text-[#d4e616] transition-colors line-clamp-2">
                      {relatedPost.title}
                    </h4>
                  </div>
                  <div className="flex items-center justify-between border-t border-white/10 pt-4">
                    <span className="text-zinc-600 font-mono text-[10px] tracking-widest uppercase">{relatedPost.readTime}</span>
                    <ArrowLeft size={14} className="text-zinc-500 group-hover:text-[#d4e616] transition-colors" />
                  </div>
                </div>
              </Link>
            ))}
