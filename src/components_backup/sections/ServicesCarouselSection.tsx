  ...
  const getCardStyles = (offset: number) => {
    ...
    if (abs === 0) {
      return {
        ...
        boxShadow: "0 20px 50px rgba(0,0,0,0.8), 0 0 40px rgba(59, 130, 246, 0.2)",
      };
    }
    if (abs === 1) {
      return {
        ...
        boxShadow: "0 10px 30px rgba(0,0,0,0.5), 0 0 0 rgba(59, 130, 246, 0)",
      };
    }
    if (abs === 2) {
      return {
        ...
        boxShadow: "0 5px 15px rgba(0,0,0,0.5), 0 0 0 rgba(59, 130, 246, 0)",
      };
    }
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden flex flex-col justify-center perspective-[2000px]">
        ...
        <div className="absolute top-24 md:top-28 inset-x-0 z-50 flex flex-col items-center text-center pointer-events-none px-6">
          <motion.h3 
            ...
            className="font-heading text-4xl md:text-5xl lg:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/40 tracking-tight drop-shadow-[0_10px_40px_rgba(0,0,0,1)] mb-5 md:mb-8"
          >
            خدماتنا التخصصية
          </motion.h3>
          <motion.div 
          ...
          <div className="relative w-full h-full flex items-center justify-center overflow-hidden z-20 px-4" dir="rtl">
            {mounted && (
              <AnimatePresence mode="popLayout" initial={false}>
                ...
                >
                  <div className="w-full h-full max-h-[65vh] relative rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        ...
        <div className="hidden md:flex absolute inset-0 z-20 items-center justify-center pointer-events-none perspective-[1200px]">
          ...
          <div className="absolute bottom-12 inset-x-0 flex justify-between items-center px-[10vw] z-50 pointer-events-none">
            <button 
              ...
              className={`w-14 h-14 rounded-full border border-white/20 backdrop-blur-xl flex items-center justify-center transition-all pointer-events-auto group ${activeIndex === mobileServices.length - 1 ? 'bg-black/20 text-white/20 cursor-not-allowed opacity-50' : 'bg-black/40 text-white/70 hover:text-blue-400 hover:border-blue-400/50 hover:bg-blue-900/30 hover:scale-110 active:scale-90 shadow-[0_0_20px_rgba(0,0,0,0.5)]'}`}
              aria-label="السابق"
            >
            ...
            <div className="flex gap-3 pointer-events-auto">
              {mobileServices.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all duration-500 ${i === activeIndex ? 'w-8 bg-accent shadow-[0_0_15px_rgba(212,230,22,0.8)]' : 'w-2 bg-white/20 hover:bg-white/50'}`}
                />
              ))}
            </div>
            ...
            <button 
              ...
              className={`w-14 h-14 rounded-full border border-white/20 backdrop-blur-xl flex items-center justify-center transition-all pointer-events-auto group ${activeIndex === 0 ? 'bg-black/20 text-white/20 cursor-not-allowed opacity-50' : 'bg-black/40 text-white/70 hover:text-blue-400 hover:border-blue-400/50 hover:bg-blue-900/30 hover:scale-110 active:scale-90 shadow-[0_0_20px_rgba(0,0,0,0.5)]'}`}
              aria-label="التالي"
            >
