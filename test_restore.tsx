...
const massiveTestimonials = [
  ...
  { text: "رعاية لا مثيل لها. الفريق الطبي يشعرك بأنك بين عائلتك." },
  ...
  { text: "فريق طبي متكامل يعمل بقلب واحد من أجل نجاح كل حالة." },
...
const TestimonialCard = forwardRef<HTMLDivElement, { text: string }>(({ text }, ref) => {
  return (
    <div 
      ref={ref}
      className="w-full px-[clamp(1.5rem,3vw,2.5rem)] py-[clamp(1.5rem,3vw,2.5rem)] rounded-3xl bg-gradient-to-b from-white/[0.08] to-transparent border border-white/5 border-t-white/20 backdrop-blur-xl flex flex-col shadow-[0_20px_40px_rgba(0,0,0,0.6)]" dir="rtl"
    >
...
export function CinematicTestimonials() {
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      ...
      // Split-HUD Parallax & Scatter Physics
      if (titleContainerRef.current) {
        const tZ = progress * -1500;
        const tY = progress * -300;
        
        // Entry animation: comes from deep space (Z: -2000) and right (X: 800)
        const entryZ = (1 - entryEased) * -2000;
        const entryX = (1 - entryEased) * 800;
        const entryRotY = (1 - entryEased) * -45;

        const finalOpacity = Math.max(0, 1 - (progress * 2)) * entryEased;

        titleContainerRef.current.style.transform = `translate3d(${entryX}px, ${tY}px, ${tZ + entryZ}px) rotateY(${entryRotY}deg)`;
        titleContainerRef.current.style.opacity = String(finalOpacity);
      }

      if (textContainerRef.current) {
        const pX = progress * -800;
        const pY = progress * 200;
        
        // Entry animation: comes from bottom (Y: 500) and left (X: -300)
        const entryY = (1 - entryEased) * 500;
        const entryX = (1 - entryEased) * -300;

        const finalOpacity = Math.max(0, 1 - (progress * 2)) * entryEased;

        textContainerRef.current.style.transform = `translate3d(${pX + entryX}px, ${pY + entryY}px, 0)`;
        textContainerRef.current.style.opacity = String(finalOpacity);
      }
    ...
    >
      ...
      >
        ...
        <div className="absolute z-50 top-0 left-0 w-full h-full pointer-events-none flex flex-col md:flex-row items-center justify-between px-6 md:px-16 lg:px-24" style={{ perspective: "1500px" }}>
          ...
          <div ref={titleContainerRef} className="w-full md:max-w-[45%] text-right mb-8 md:mb-0 will-change-transform" style={{ transformStyle: "preserve-3d" }}>
            <div className="border border-white/20 rounded-full px-5 py-1.5 uppercase text-xs tracking-[0.2em] font-medium text-white/60 mb-6 inline-block backdrop-blur-md shadow-[0_0_30px_rgba(192,232,22,0.1)]">
              قصص النجاح
            </div>
            <h2 className="font-heading text-3xl md:text-5xl lg:text-[4rem] font-bold text-white drop-shadow-2xl leading-tight">
              حملوا أحلامهم إلينا،<br/>وعادوا بأطفالهم
            </h2>
          </div>

          {/* Left Side (RTL End) - Description */}
          <div ref={textContainerRef} className="w-full md:max-w-[35%] will-change-transform">
            <div className="bg-[#000814]/60 backdrop-blur-2xl p-6 md:p-10 rounded-[2rem] border border-white/10 shadow-[0_40px_80px_rgba(0,0,0,0.8)] relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#c0e816] blur-[80px] opacity-20 -translate-y-1/2 translate-x-1/2" />
              <p className="font-sans text-zinc-300 text-base md:text-lg drop-shadow-md font-medium leading-relaxed relative z-10">
        ...
        <div className="absolute bottom-0 left-0 w-full h-[30vh] bg-gradient-to-t from-black via-black/60 to-transparent z-40 pointer-events-none" />
          ...
          <div className="absolute inset-0 z-20 pointer-events-none overflow-hidden flex justify-center items-center">
            ...
            <div 
              ...
              className="w-[300vw] md:w-[250vw] h-[300vh] flex justify-center gap-4 md:gap-6 -rotate-[8deg] will-change-transform"
            >
              
              {/* Column 1 (Moves UP) */}
              <div ref={col1Ref} className="flex-1 flex flex-col gap-4 md:gap-6 pt-[100px] will-change-transform">
                {col1Items.map((t, i) => (
              ...
              {/* Column 2 (Moves DOWN) */}
              <div ref={col2Ref} className="flex-1 flex flex-col gap-4 md:gap-6 pt-[400px] will-change-transform">
                {col2Items.map((t, i) => (
              ...
              {/* Column 3 (Moves UP) */}
              <div ref={col3Ref} className="flex-1 flex flex-col gap-4 md:gap-6 pt-[200px] will-change-transform">
                {col3Items.map((t, i) => (
              ...
              {/* Column 4 (Moves DOWN) */}
              <div ref={col4Ref} className="flex-1 flex flex-col gap-4 md:gap-6 will-change-transform">
                {col4Items.map((t, i) => (
              ...
              {/* Column 5 (Moves UP) */}
              <div ref={col5Ref} className="flex-1 flex flex-col gap-4 md:gap-6 pt-[300px] will-change-transform">
                {col5Items.map((t, i) => (
              ...
              {/* Column 6 (Moves DOWN) */}
              <div ref={col6Ref} className="flex-1 flex flex-col gap-4 md:gap-6 pt-[200px] will-change-transform">
                {col6Items.map((t, i) => (
              ...
              {/* Column 7 (Moves UP) */}
              <div ref={col7Ref} className="flex-1 flex flex-col gap-4 md:gap-6 pt-[500px] will-change-transform">
                {col7Items.map((t, i) => (
